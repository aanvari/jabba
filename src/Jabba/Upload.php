...
class Upload
{
    ...
    public function handle() 
    {
        ...
        while (file_exists($path . File\Image::getPathByName($uniqueId, 3, true) . $uniqueId . '.' . $ext)) {
            $uniqueId = File::getRandomString($length);
        }

        $path .= File\Image::getPathByName($uniqueId, 3, true) . $uniqueId . '.' . $ext;

        if ($this->getHandler()->save($path)) {            
